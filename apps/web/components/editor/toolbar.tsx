...
export function Toolbar({
  ...
  const disabled = !editor || !state || state.inLocked;
  ...
  const items: Array<{ label: string; icon: React.ReactNode; active?: boolean; run: () => void; disabled?: boolean }> = [
    ...
    { label: `Heading 1 (${mod}+Alt+1)`, icon: <Heading1 size={16} />, active: state?.h1, run: () => editor?.chain().focus().toggleHeading({ level: 1 }).run() },
    { label: `Heading 2 (${mod}+Alt+2)`, icon: <Heading2 size={16} />, active: state?.h2, run: () => editor?.chain().focus().toggleHeading({ level: 2 }).run() },
    { label: `Heading 3 (${mod}+Alt+3)`, icon: <Heading3 size={16} />, active: state?.h3, run: () => editor?.chain().focus().toggleHeading({ level: 3 }).run() },
    { label: `Bullet list (${mod}+Shift+8)`, icon: <List size={16} />, active: state?.bullet, run: () => editor?.chain().focus().toggleBulletList().run() },
    { label: `Numbered list (${mod}+Shift+7)`, icon: <ListOrdered size={16} />, active: state?.ordered, run: () => editor?.chain().focus().toggleOrderedList().run() },
  ];
      ...
      <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-1 px-3 py-1.5 sm:px-4">
        {items.map((it) => (
          <Tip key={it.label} label={it.label}>
            <Button variant="ghost" size="icon" active={!!it.active} disabled={disabled} onClick={it.run} aria-label={it.label}>
              {it.icon}
            </Button>
          </Tip>
        ))}
        ...
        {lockName && (
          <span className="ml-auto text-[12px] text-muted" role="status">
            Paragraph is being edited by {lockName}
          </span>
        )}
      </div>
    </div>
