...
Conflict Resolution — Concurrent edits, including edits to the same text at the same time, always merge correctly. Last-Write-Wins is not used.
Live Presence — Every connected user's cursor position and text selection is visible to other users in real time.
Persistence & Recovery — A dropped network connection does not result in data loss. Edits made while offline are preserved and merged correctly on reconnect.
Features
...
└── README.md
Roadmap
Core CRDT synchronization between clients
Live cursor and selection presence
Offline persistence and reconnect recovery
Role-based permission enforcement across API and WebSocket layers
Network partition simulation for demonstrating recovery behavior
Extended presence indicators (active block, editing status)
License
